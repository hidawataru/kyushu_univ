...
/*** DB接続 ***/
  ...
  try{
	...
	$pdo = new PDO("pgsql:host=$DBHOST;port=$DBPORT;dbname=$DBNAME;user=$DBUSER;password=$DBPASS");
          ...
          //SQL作成
          $sql = 'select * from fkt_kiki."M_Kiki"';
...
if  ( isset($_POST['register']) ) {
	...
	if( $my_name == '' || $sect == '') { //名前か所属が空欄だったら
		...
		if( $sbm_flg == true ) { //フラグが立ってたら登録できない
			...
			//登録処理
			$sql = $pdo->prepare(
				'INSERT INTO fkt_kiki.rsv_timetable
	...
	$results = $pdo->prepare(	'SELECT kwd FROM fkt_kiki.rsv_timetable WHERE id = :id' );
	...
	if ( $ipt_kwd === $kwd || $ipt_kwd === $master_key ) {
		$sql = $pdo->prepare( 'DELETE FROM fkt_kiki.rsv_timetable WHERE id = :id' );
		$sql->bindValue(':id', $id, PDO::PARAM_INT);
		$rsl = $sql->execute(); //実行
		if ( $rsl == false ){
			$log1 = '<p>削除に失敗しました。</p>';
