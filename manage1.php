    ...
    <!-- PostGreSQLへの接続 -->
       <?php
        ...
        try{
          ...
          //SQL作成
          $sql = 'select * from fkt_kiki."rsv_timetable"';
        ...
        <div class="container">
                ...
                <tr>
                  ...
                  <td>
                    <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#testModal"
                        data-cpt="<?php echo $row['cpt_name']; ?>" 
                        data-name="<?php echo $row['name']; ?>" 
                        data-yoyakuday="<?php echo date('Y年m月d日', strtotime($row['time_st'])); ?>" 
                        data-yoyakutime="<?php echo date('H時i分', strtotime($row['time_st'])); ?>" 
                        >
                    削除</button>
                  </td>
                </tr>
            ...
            <script>
                $('#testModal').on('show.bs.modal', function (event) {
                    ...
                    // data-***の部分を取得
                    var sampledata = button.data('cpt');
                    var modal = $(this);
                    // モーダルに取得したパラメータを表示
                    // 以下ではh5のモーダルタイトルのクラス名を取得している
                    modal.find('.modal-cpt').val(sampledata);
                })
            </script> 
